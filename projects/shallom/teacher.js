(function(){
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    function renderGrades(){ const tbody = document.getElementById('gradesBody'); if(!tbody) return; tbody.innerHTML=''; const gs = loadGrades().filter(x=> x.createdBy===user.username || x.teacher===user.name ); if(gs.length===0){tbody.innerHTML='<tr><td colspan="7">No grades yet.</td></tr>';return} gs.forEach((g,i)=>{ const tr=document.createElement('tr'); tr.innerHTML=`<td>${g.id||i+1}</td><td>${g.studentId}</td><td>${g.class}</td><td>${g.subject}</td><td>${g.exam}</td><td>${g.grade}</td><td>${g.teacher||g.createdBy}</td>`; tbody.appendChild(tr); }) }
    const gradeFormEl = document.getElementById('gradeForm');
    if (gradeFormEl) {
      gradeFormEl.addEventListener('submit', e=>{ e.preventDefault(); const studentId=document.getElementById('gradeStudentId').value.trim(); const cls=document.getElementById('gradeClass').value.trim(); const subject=document.getElementById('gradeSubject').value.trim(); const exam=document.getElementById('gradeExam').value.trim(); const grade=document.getElementById('gradeValue').value.trim(); if(!studentId||!cls||!subject||!exam||!grade) return notify('Fill all fields'); let gs = loadGrades(); const id = 'G'+String(gs.length+1).padStart(3,'0'); const g = { id, studentId, class:cls, subject, exam, grade, teacher: user.name, createdBy: user.username, time: new Date().toLocaleString()}; gs.push(g); saveData('grades', gs); addActivity('📝', `Saved grade for ${studentId} (${subject})`); renderGrades(); try{ if(typeof loadGradesForMain === 'function') loadGradesForMain(); }catch(e){} gradeFormEl.reset(); });
    }
    renderGrades();
