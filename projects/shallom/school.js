...
function loadAllData() {
    ...
    // Load grades for main dashboard (if present)
    loadGradesForMain();
...
function renderClasses() {
    ...
    // If user selected compact dropdown mode, render a single select + detail pane
    ...
    if (viewMode === 'dropdown') {
        ...
        // helper to render the compact class card in the detail box
    ...
    // Render each form level as a collapsible group (compact / scrollable)
    sortedForms.forEach(formLevel => {
        ...
        // Create grid for classes inside the collapsible body
...
function promptAddGrade(studentId) {
    ...
    if (saveData('grades', grades)) {
        ...
        // also refresh main dashboard grades table if present
        loadGradesForMain();
    } else {
...
// Show a grade-entry prompt from the main/headmaster dashboard
function showGradeForm() {
    const classes = getData('classes') || [];
    const students = getData('students') || [];

    if (!Array.isArray(classes) || classes.length === 0) {
        // fallback to selecting a student from all students
...
// Render grades table on the main dashboard
function loadGradesForMain() {
    const tbody = document.getElementById('mainGradesTableBody');
    if (!tbody) return; // grades section might not be present
...
// Show Add Timetable entry form (simple prompt-based input for main dashboard)
...
// --- Attendance (read-only; sourced from teacher dashboard) ---
function renderStudentAttendance(studentId) {
    const panel = document.getElementById('profileAttendancePanel');
    if (!panel) return;

    // Collect teacher-marked attendance and any legacy profile attendance (compat)
    ...
    // Merge and sort (teacher records first if duplicate date)
...
// NOTE: attendance is controlled by teachers' dashboard — no add/delete from head profile


// --- Parent Contacts (now supports phone numbers) ---
...
// --- Fees panel (read-only — mark payments only from Fees section) ---
...
// Allow clicking outside student profile modal to close
document.addEventListener('click', function(e) {
    const modal = document.getElementById('studentProfileModal');
    if (!modal) return;
...
function showAddStudentForm() {
    // open modal-based student form (class list is taken from Classes management)
    openStudentModal();
}
...
// Allow clicking outside student modal content to close
document.addEventListener('click', function(e) {
    const modal = document.getElementById('studentModal');
    if (!modal) return;
...
function deleteStudentRecord(button, studentId) {
    if (confirm('Are you sure you want to delete this student?')) {
        ...
        // Remove parent contact logs and attendance records for the student
        let contacts = getData('parentContacts') || [];
        ...
        // Also remove teacher-marked attendance entries
...
// showGradeForm removed from headmaster UI — grade entry is handled from teacher dashboard or class details.
...
function populateClassFilter() {
    ...
    // keep student add-class select in sync with classes
    populateStudentClassSelect();
}
...
// stray timetable prompt snippet removed (function is defined earlier)
